...
const ExitMeetingPage: React.FC = () => {
  ...
  const [editingItem, setEditingItem] = useState<ExitMeeting | null>(null);
  ...
  const filteredData = useMemo(() => {
    let filtered = [...EXIT_MEETING_DATA];
  ...
  const handleView = (item: ExitMeeting) => {
    setEditingItem(item);
    setDialogMode('view');
    setIsDialogOpen(true);
  };

  const handleEdit = (item: ExitMeeting) => {
    setEditingItem(item);
    setDialogMode('edit');
    setIsDialogOpen(true);
  };

  const handleSave = (data: Partial<ExitMeeting>) => {
